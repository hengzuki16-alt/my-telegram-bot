...
# ១. មុខងារស្វាគមន៍ /start
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [["🛍️ ធ្វើការកម្មង់ (Order)"]]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=True)
    
    await update.message.reply_text(
        "សួស្តី! សូមចុចប៊ូតុងខាងក្រោមដើម្បីចាប់ផ្តើមទិញទំនិញ៖",
...
async def get_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    instruction_text = (
        ...
        "4. រួចចុចលើពាក្យ 'Send This Location' ឬ 'Send Selected Location'"
    )
    
    await update.message.reply_text(instruction_text)
    return LOCATION

# ៦. ទទួលទីតាំងដែលភ្ញៀវដៅផ្ញើមក រួចបង្ហាញព័ត៌មានឱ្យផ្ទៀងផ្ទាត់
async def get_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.location:
        ...
        await update.message.reply_text("⚠️ សូមចុចលើសញ្ញា 📎 រួចរើសយក 'Location' ដើម្បីដៅទីតាំងលើផែនទីផ្ញើមកកាន់ Bot។")
        return LOCATION

# ៧. ជំហានចុងក្រោយ៖ ផ្ញើព័ត៌មានទៅកាន់ Admin
async def confirm_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_chat_id = 799801965  # ID របស់ Admin
    
    try:
        # ផ្ញើសារលម្អិតទៅ Admin
        await context.bot.send_message(
            chat_id=admin_chat_id,
            text=f"🔔 [Order ថ្មី -ឥវ៉ាន់មិនទាន់បង់ប្រាក់]\n👤 ឈ្មោះ: {name}\n📞 លេខទូរស័ព្ទ: {phone}\n🛍️ ទំនិញ: {product}\n📍 លីងផែនទី Google Maps: {google_maps_link}"
        )
        # ផ្ញើ Map Pin ទៅ Admin មើលលើទូរស័ព្ទ
    ...
    await update.message.reply_text("✅ ការកម្មង់របស់អ្នកត្រូវបានបញ្ជូនទៅកាន់ក្រុមការងាររួចរាល់ហើយ! យើងនឹងទាក់ទងទៅអ្នកក្នុងពេលឆាប់ៗ។", reply_markup=reply_markup)
...
# =======================================================
# ដំណើរការប្រព័ន្ធ Bot
# =======================================================
def main():
    # ប្រើប្រាស់ Token របស់អ្នក
    app = Application.builder().token("8803711682:AAFMowMRnXeqTkPKrs2AmcIhNHXxdHky5us").build()
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            LOCATION: [
                # ចាប់យកតែសារដែលជាប្រភេទ LOCATION ដែលភ្ញៀវដៅផ្ញើមកប៉ុណ្ណោះ
    ...
    print("Bot កំពុងដំណើរការដោយរលូនហើយ...")
    app.run_polling()
